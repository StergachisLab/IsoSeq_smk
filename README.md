...
When running the test with a single sample, it is necessary to exclude the final rule, isoranker_analysis, as this step requires multiple samples and additional files to execute correctly. To achieve this, we add the flag `run_isoranker: false` on our config yaml file. If that flag is not present, the following command might also help:
